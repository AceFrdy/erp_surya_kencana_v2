...
const SearchCabang = () => {
    const { isOpen, type, onClose, cabang, setGet } = useModal();

    const isModalOpen = isOpen && type === 'search-cabang';
                        ...
                        >
                            <Dialog.Panel as="div" className="panel border-0 p-5 rounded-lg overflow-hidden my-8 w-full max-w-5xl text-black dark:text-white-dark">
                                <div className="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between">
                                    <div className="text-lg font-bold">Pilih Tujuan</div>
                                </div>
                                        ...
                                        columns={[
                                            {
                                                ...
                                                sortable: true,
                                                render: (e) =>
                                                    search.length > 0 ? cabang.filter((item) => item.branch_name?.toLowerCase().includes(search.toLowerCase())).indexOf(e) + 1 : cabang.indexOf(e) + 1,
                                            },
                                            {
